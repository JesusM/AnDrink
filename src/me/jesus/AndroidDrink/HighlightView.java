...
class HighlightView {
    ...
    void handleMotion(int edge, float dx, float dy) {
        ...
        if (edge == GROW_NONE) {
            ...
            float xDelta = dx * (mCropRect.height() / r.height());
            float yDelta = dy * (mCropRect.width() / r.width());
            growBy((((edge & GROW_TOP_EDGE) != 0) ? -1 : 1) * xDelta,
                    (((edge & GROW_LEFT_EDGE) != 0) ? 1 : -1) * yDelta);
        }
    }
